...
int main(int argc, char** argv) {
    ...
    // float Scale_C = 0.779934283;
    // float Scale_S = 0.729590164;

    //  2024-Feb-06th Re-Calib integral result
    float Equ_3D_C     = 0.0000755958;
    ...
    float Scale_C = 0.832258065;
    float Scale_S = 0.769483467;
    ...
    TH1F* Hist_C_Scaled_2   = new TH1F("C_response_Scaled_2", "C_response_Scaled_2;GeV;Evt", 250, -5, 20); // Regard 3D as single tower 3D response = (MCP + Wings) * 3D equ. const.
    TH1F* Hist_S_Scaled_2   = new TH1F("S_response_Scaled_2", "S_response_Scaled_2;GeV;Evt", 250, -5, 20);
    TH1F* Hist_Sum_Scaled_2 = new TH1F("Sum_response_Scaled_2", "Sum_response_Scaled_2;GeV;Evt", 500, -10, 40);
    ...
    TH1F* Hist_C_Scaled_2_cut   = new TH1F("C_response_Scaled_2_cut", "C_response_Scaled_2_cut;GeV;Evt", 250, -5, 20);
    TH1F* Hist_S_Scaled_2_cut   = new TH1F("S_response_Scaled_2_cut", "S_response_Scaled_2_cut;GeV;Evt", 250, -5, 20);
    TH1F* Hist_Sum_Scaled_2_cut = new TH1F("Sum_response_Scaled_2_cut", "Sum_response_Scaled_2_cut;GeV;Evt", 500, -10, 40);
    ...
    for (int iEvt = 0; iEvt < totalEntry; iEvt++) {
        ...
        Hist_C_Scaled_2->Fill(Equalized_C_2 * Scale_C);
        Hist_S_Scaled_2->Fill(Equalized_S_2 * Scale_S);
        Hist_Sum_Scaled_2->Fill( (Equalized_C_2 * Scale_C) + (Equalized_S_2 * Scale_S));
        ...
        Hist_C_Scaled_2_cut->Fill(Equalized_C_2 * Scale_C);
        Hist_S_Scaled_2_cut->Fill(Equalized_S_2 * Scale_S);
        Hist_Sum_Scaled_2_cut->Fill( (Equalized_C_2 * Scale_C) + (Equalized_S_2 * Scale_S));
    ...
    // Save the outputs
    std::string outFile = "./Energy/Energy_EvtPed_Run_" + std::to_string(runNum) + "_24Feb06ReCalib.root";
    ...
    Hist_C_Scaled_2->Write();
    Hist_S_Scaled_2->Write();
    Hist_Sum_Scaled_2->Write();
    ...
    Hist_C_Scaled_2_cut->Write();
    Hist_S_Scaled_2_cut->Write();
    Hist_Sum_Scaled_2_cut->Write();
