...
// This script is for measuring energy (with scale factor)
// Calculate scale factor before running this script!
//
// 1. Compile the script
// ./compile energy
//
// 2. Run the compiled script
// ./energy <run number> <max entry>
// Ex) ./energy 4234 -1
...
int main(int argc, char** argv) {
    ...
    float Scale_C = 1.142204455;
    float Scale_S = 1.096791884;
    ...
    TH1F* Hist_C_E_Scaled   = new TH1F("C_Energy_Scaled", "C_Energy_Scaled;GeV;Evt", 250, -5, 20); // Regard 3D as single tower 3D response = (MCP + Wings) * 3D equ. const.
    TH1F* Hist_S_E_Scaled   = new TH1F("S_Energy_Scaled", "S_Energy_Scaled;GeV;Evt", 250, -5, 20);
    TH1F* Hist_Sum_E_Scaled = new TH1F("Sum_Energy_Scaled", "Sum_Energy_Scaled;GeV;Evt", 500, -10, 40);
    ...
    TH1F* Hist_C_E_Scaled_cut   = new TH1F("C_Energy_Scaled_cut", "C_Energy_Scaled_cut;GeV;Evt", 250, -5, 20); // Regard 3D as single tower 3D response = (MCP + Wings) * 3D equ. const.
    TH1F* Hist_S_E_Scaled_cut   = new TH1F("S_Energy_Scaled_cut", "S_Energy_Scaled_cut;GeV;Evt", 250, -5, 20);
    TH1F* Hist_Sum_E_Scaled_cut = new TH1F("Sum_Energy_Scaled_cut", "Sum_Energy_Scaled_cut;GeV;Evt", 500, -10, 40);
    ...
    for (int iEvt = 0; iEvt < totalEntry; iEvt++) {
        ...
        Hist_C_E_Scaled->Fill(Calibrated_C * Scale_C);
        Hist_S_E_Scaled->Fill(Calibrated_S * Scale_S);
        Hist_Sum_E_Scaled->Fill( (Calibrated_C * Scale_C) + (Calibrated_S * Scale_S));
        ...
        Hist_C_E_Scaled_cut->Fill(Calibrated_C * Scale_C);
        Hist_S_E_Scaled_cut->Fill(Calibrated_S * Scale_S);
        Hist_Sum_E_Scaled_cut->Fill( (Calibrated_C * Scale_C) + (Calibrated_S * Scale_S));
    ...
    std::string outFile = "./Energy/Energy_EvtPed_Run_" + std::to_string(runNum) + ".root";
    ...
    Hist_C_E_Scaled->Write();
    Hist_S_E_Scaled->Write();
    Hist_Sum_E_Scaled->Write();
    ...
    Hist_C_E_Scaled_cut->Write();
    Hist_S_E_Scaled_cut->Write();
    Hist_Sum_E_Scaled_cut->Write();
