...
int main(int argc, char** argv) {
    ...
    for (int iEvt = 0; iEvt < totalEntry; iEvt++) {
        ...
        // float IntADC_3D_W1_C  = GetIntWithPed(Wave_3D_C.at(1), 130, 275, Ped_3D_W1_C );
        // float IntADC_3D_W2_C  = GetIntWithPed(Wave_3D_C.at(2), 145, 260, Ped_3D_W2_C );
        // float IntADC_3D_W3_C  = GetIntWithPed(Wave_3D_C.at(3), 140, 250, Ped_3D_W3_C );

        // float IntADC_3D_mid_S = GetIntWithPed(Wave_3D_S.at(0), 150, 330, Ped_3D_mid_S);
        // float IntADC_3D_W1_S  = GetIntWithPed(Wave_3D_S.at(1), 165, 325, Ped_3D_W1_S );
        // float IntADC_3D_W2_S  = GetIntWithPed(Wave_3D_S.at(2), 160, 305, Ped_3D_W2_S );
        // float IntADC_3D_W3_S  = GetIntWithPed(Wave_3D_S.at(3), 150, 295, Ped_3D_W3_S );

        // float IntADC_HS_W_C  = GetIntWithPed(Wave_HeatSink_C.at(0), 135, 280, Ped_HW_C);
        // float IntADC_HS_1_C  = GetIntWithPed(Wave_HeatSink_C.at(1), 150, 325, Ped_H1_C);
        // float IntADC_HS_2_C  = GetIntWithPed(Wave_HeatSink_C.at(2), 145, 325, Ped_H2_C);
        // float IntADC_HS_3_C  = GetIntWithPed(Wave_HeatSink_C.at(3), 145, 300, Ped_H3_C);

        // float IntADC_HS_W_S  = GetIntWithPed(Wave_HeatSink_S.at(0), 140, 335, Ped_HW_S);
        // float IntADC_HS_1_S  = GetIntWithPed(Wave_HeatSink_S.at(1), 160, 330, Ped_H1_S);
        // float IntADC_HS_2_S  = GetIntWithPed(Wave_HeatSink_S.at(2), 155, 325, Ped_H2_S);
        // float IntADC_HS_3_S  = GetIntWithPed(Wave_HeatSink_S.at(3), 170, 330, Ped_H3_S);

        // float IntADC_L_1_C  = GetIntWithPed(Wave_Lego_C.at(0), 155, 330, Ped_L1_C);
        // float IntADC_L_2_C  = GetIntWithPed(Wave_Lego_C.at(1), 215, 370, Ped_L2_C);
        // float IntADC_L_3_C  = GetIntWithPed(Wave_Lego_C.at(2), 145, 300, Ped_L3_C);
        // float IntADC_L_4_C  = GetIntWithPed(Wave_Lego_C.at(3), 150, 395, Ped_L4_C);

        // float IntADC_L_1_S  = GetIntWithPed(Wave_Lego_S.at(0), 170, 340, Ped_L1_S);
        // float IntADC_L_2_S  = GetIntWithPed(Wave_Lego_S.at(1), 220, 405, Ped_L2_S);
        // float IntADC_L_3_S  = GetIntWithPed(Wave_Lego_S.at(2), 155, 330, Ped_L3_S);
        // float IntADC_L_4_S  = GetIntWithPed(Wave_Lego_S.at(3), 160, 340, Ped_L4_S);
    ...
    // Save the outputs
    std::string outFile = "./IntADC/IntADC_EvtPed_Run_" + std::to_string(runNum) + "_24Feb06ReCalib.root";
