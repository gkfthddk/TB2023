...
public:
  ...
  {
    fReaderWave = new TBread<TBwaveform>(fRunNum, fMaxEvent, -1, "/eos/user/s/sungwon/public/TB2023_data", std::vector<int>{1, 2, 3, 4});
    fReaderFast = nullptr;
  }
  void SetReader(TBfastmode mode)
  {
    fReaderFast = new TBread<TBfastmode>(fRunNum, fMaxEvent, -1, "/eos/user/s/sungwon/public/TB2023_data", std::vector<int>{1, 2, 3, 4});
    fReaderWave = nullptr;
  }
